...
def mock_send_email(trial_request_id: int) -> None:
    # потом заменишь на celery task
    print(f"Send email for trial request #{trial_request_id}")
...
def create_trial_lesson_request(*, student, teacher, start_at, end_at, student_note=''):
    with transaction.atomic():
        ...
        transaction.on_commit(
            lambda: mock_send_email(trial_request_id)
        )
